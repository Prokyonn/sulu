...
namespace Sulu\Article\Tests\Unit\Infrastructure\Sulu\Reference;
...
class ArticleReferenceRefresherTest extends TestCase
{
    use ProphecyTrait;
    use SetGetPrivatePropertyTrait;

    private ArticleReferenceRefresher $refresher;
    ...
    private ObjectProphecy $articleDimensionContentRepository;
    ...
    protected function setUp(): void
    {
        ...
        /** @var ObjectProphecy<EntityRepository<ArticleDimensionContentInterface>> $prophecy */
    ...
    {
        $this->assertSame(Article::RESOURCE_KEY, ArticleReferenceRefresher::getResourceKey());
    }
    ...
    public function testRefreshReturnsGenerator(): void
    {
        ...
        $queryBuilder->orderBy('dimensionContent.article', 'ASC')->willReturn($queryBuilder);
        $queryBuilder->getQuery()->willReturn($query->reveal());

        $query->toIterable()->willReturn(new \ArrayIterator([]));

        $this->articleDimensionContentRepository->createQueryBuilder('dimensionContent')
    ...
    public function testRefreshWithFilter(): void
    {
        ...
        $filter = ['resourceId' => '123', 'resourceKey' => 'articles', 'locale' => 'en', 'stage' => 'live'];
        ...
        $queryBuilder->setParameter('version', DimensionContentInterface::CURRENT_VERSION)->willReturn($queryBuilder);
        $queryBuilder->orderBy('dimensionContent.article', 'ASC')->willReturn($queryBuilder);
        $queryBuilder->join('dimensionContent.article', 'article', \Doctrine\ORM\Query\Expr\Join::WITH, 'article.uuid = :resourceId')->willReturn($queryBuilder);
