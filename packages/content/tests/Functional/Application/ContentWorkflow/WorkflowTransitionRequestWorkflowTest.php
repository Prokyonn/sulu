...
class WorkflowTransitionRequestWorkflowTest extends SuluTestCase
{
    ...
    /**
     * A rejection is a reviewer's vote on the request, so the `reject` content transition leaves the
     * request open: withdrawing it stays the separate, deliberate `cancel_review`.
     */
    public function testRejectLeavesTheRequestOpen(): void
    {
        $example = $this->createExampleAtDraft();
        $dimensionAttributes = ['stage' => DimensionContentInterface::STAGE_DRAFT, 'locale' => 'en'];
        ...
        $this->assertSame(WorkflowTransitionRequestStatusEnum::PENDING, $this->resolveRequestStatus($rejectedRequest));
        $this->assertNotNull($rejectedRequest->getActiveKey(), 'Only a cancel closes a request.');
    }
