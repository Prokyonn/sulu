...
final class ValidateWorkflowTransitionRequestMessageHandler
{
    ...
    public function __invoke(ValidateWorkflowTransitionRequestMessage $message): void
    {
        ...
        foreach ($request->getDecisions() as $decision) {
            ...
            if (null === $entry) {
                $result = ValidationResult::reject(DecisionMessage::text(\sprintf("Check failed: validator '%s' is not registered", $validatorKey)));
            } else {
                try {
                    ...
                    $result = ValidationResult::reject(DecisionMessage::translated('sulu_content.workflow_transition_request.check_errored'));
                }
            }
