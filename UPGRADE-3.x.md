...
- Publishing through the API takes `live`, or `edit` together with an approved active request, for
  content a request workflow covers and for pages, articles and snippets: a role with `edit` alone gets
  a 403 there. `reject` and `reject_draft` take `review`. A call with no authenticated user publishes on
