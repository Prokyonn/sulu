 ...
 * fabricates them in setUp and tears them down again. Both the fabrication and the row inserts
 * use the DBAL Schema API and QueryBuilder so the test runs on every supported database.
 */
class Version20260429120400Test extends SuluTestCase
{
    ...
    private function applySchemaChange(callable $modifier): void
    {
        ...
        $sqls = $fromSchema->getMigrateToSql($toSchema, $this->connection->getDatabasePlatform());
        foreach ($sqls as $sql) {
